				...
				function i(e, t, r) {
					...
					for (
						...
						if ('string' == typeof g) _ += a(l(g));
							...
							else _ += '(?:'.concat(E).concat(R, ')').concat(g.modifier);
						}
					}
					if (void 0 === c || c)
						(i || (_ += ''.concat(h, '?')),
		...
		function d(e, t, r, n, a, l = { names: {}, intercepted: {} }) {
			...
			(0, c.removeTrailingSlash)(e).slice(1).split('/'))) {
				...
				if (
					...
					(m.push(r),
						g.push(`/${d[1]}:${l.names[t] ?? n}${o ? (i ? '*' : '+') : ''}`),
						(l.names[t] ??= n));
				} else if (d && d[2]) {
					n &&
						d[1] &&
						(m.push(`/${(0, u.escapeStringRegexp)(d[1])}`), g.push(`/${d[1]}`));
					let {
							key: t,
							pattern: r,
							cleanedKey: o,
					...
					(n && d[1] && (s = s.substring(1)),
						m.push(s),
						g.push(`/:${l.names[t] ?? o}${i ? (c ? '*' : '+') : ''}`),
						(l.names[t] ??= o));
				} else (m.push(`/${(0, u.escapeStringRegexp)(f)}`), g.push(`/${f}`));
				r &&
					d &&
					d[3] &&
					(m.push((0, u.escapeStringRegexp)(d[3])), g.push(d[3]));
			}
			return {
